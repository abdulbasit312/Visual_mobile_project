...
class SpatialOperation(nn.Module):
    ...
    def forward(self, x):
        x=self.skip_add.mul(x,self.block(x))
        return x 
...
class AdditiveTokenMixer(nn.Module):
    ...
    def forward(self, x):
        ...
        q = self.oper_q(q)
        k = self.oper_k(k)
        r = self.skip_add.add(q,k)
        s = self.dwc(r)
        t = self.skip_add.mul(s,v)
        out = self.proj(t)
...
class AdditiveBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., attn_bias=False, drop=0., drop_path=0.,
        ...
        self.skip_add = nn.quantized.FloatFunctional()
    def forward(self, x):
        x = self.skip_add.add(x,self.local_perception(x))
        x = self.skip_add.add(x,self.drop_path(self.attn(self.norm1(x))))
        x = self.skip_add.add(x,self.drop_path(self.mlp(self.norm2(x))))
        return x
    ...
    def forward(self, x):
        ...
        if self.dist:
            ...
            if not self.training:
                cls_out = self.skip_add.add(cls_out[0],cls_out[1])
        else:

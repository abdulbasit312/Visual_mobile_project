...
def get_args_parser():
    ...
    parser.add_argument('--warmup_start_lr', type=float, default=1e-4, metavar='LR',
    ...
    parser.add_argument('--calculate_flops', type=str2bool, default=False)
    parser.add_argument('--convert_to_onnx', type=str2bool, default=False)
    return parser
...
def main(args):
    ...
    if args.calculate_flops:
        ...
        print("FLOPS data")
        flops,macs,params=calculate_flops(model, (1, 3, args.input_size, args.input_size),output_as_string=True,output_precision=4)
        print(f"flops {flops} , macs {macs} , params {params}")
        return
    
    if args.convert_to_onnx:
        print(f"Convert to onnx")
        model.eval()
        example_inputs = (torch.randn(1, 3, args.input_size, args.input_size).to(device),)
        input_names = ["input"]
        output_names = ["output"]
        input_params = (
        torch.rand([1, 3, args.input_size, args.input_size]).to(device))
        torch.onnx.export(model,
                  input_params,
                  os.path.join(args.output_dir,"output.onnx"),
